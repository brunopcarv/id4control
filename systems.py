...
class LorenzEquations():
	...
	def dynamics(self, x, u, eta, dt): #with additive noise eta
		...
		x0 = x[0] + (self.sigma * (x[1]*x[0]) + u + self.H[0]*next(eta))*dt
		x1 = x[1] + (x[0] * (self.rho-x[2]) - x[1]  + self.H[1]*next(eta))*dt
		x2 = x[2] + (x[0]*x[1] - self.beta*x[2] + self.H[2]*next(eta))*dt
		return np.array([x0, x1, x2]).T
...
class InvertedPendulum():
	...
	def dynamics(self, x, u, eta, dt): #with additive noise eta
		...
		num0 = -self.b*x[1]
		num1 = (self.m*self.l)**2*self.GRAVITY*np.sin(x[2])*np.cos(x[2])/den
		...
		x3 = x[3] + ((-self.m*self.l*(x1*np.cos(x[2])+self.GRAVITY*np.sin(x[2])))/den + self.H[3]*next(eta))*dt
