...
class PredatorPreyPlant():
	...
	def dynamics(self, x, u, dt):
		x0 = x[0]+ (self.a*x[0] - self.b*x[0]*x[1] + u**2)*dt
		x1 = x[1]+ (-self.c*x[1] + self.d*x[0]*x[1])*dt
...
class InvertedPendulum():
	...
	def __init__(self, M=0.5, m=0.2, b=0.1, I=0.006, l=0.3):
		self.M = M
		self.m = m
		self.b = b
		self.I = I
	...
	def dynamics(self, x, u, dt):
		...
		x0 = x[0] + x[1]*dt 
		x1 = x[1] + (num0+num1+num2+u)/bigden*dt
		x2 = x[2] + x[3]*dt
		x3 = x[3] + (-self.m*self.l*(x1*np.cos(x[2])+GRAVITY*np.sin(x[2])))/den*dt
